...
class FusionNet(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=channel, out_channels=spectral_num, kernel_size=3, stride=1, padding=1,
                               bias=True)
        ...
        init_weights(self.backbone, self.conv1, self.conv2)   # state initialization, important!

    def forward(self, x, y):  # x= hp of ms; # Bsx8x64x64 y = hp of pan # Bsx1x64x64
        h, w = y.shape[2], y.shape[3]
        y_8 = y.repeat_interleave(8, 0).reshape(-1, 8, h, w)
