...
from data import TrainValDatasetH5
from model import DeRain, summaries
import numpy as np
...
criterion = nn.MSELoss(size_average=True).to(device) ## Define the Loss function L2Loss
...
def train(training_data_loader, validate_data_loader,start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs, 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1): # 100  3
            ...
            # ms_hp Nx8x16x16
            # pan_hp Nx1x64x64
            img, gt = batch['img'].to(device), batch['gt'].to(device)
            optimizer.zero_grad()  # fixed

            output = model(img)  

            loss = criterion(output, gt)  # compute loss
...
if __name__ == "__main__":
    train_set = TrainValDatasetH5('derain_small\Rain200L_train_chunks.h5')  # creat data for training   # 100
    training_data_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=batch_size, shuffle=True,  # 3 32
                                      pin_memory=True, drop_last=True)  # put training data to DataLoader for batches

    validate_set = TrainValDatasetH5('derain_small\Rain200L_val_chunks.h5')  # creat data for validation
